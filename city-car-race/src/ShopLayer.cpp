...
bool ShopLayer::init()
{
	...
	Sprite* bg = Sprite::create("pause/MenuBg.png");
	bg->setPosition(Point(270,480));
	this->addChild(bg,1);

	for(int i=0;i<2;i++)
	{
		itemSprite[i] = Sprite::create("Shop/ShopBottom.png");
		itemSprite[i]->setPosition(Point(195+i*150,650));
		this->addChild(itemSprite[i],2);

		Sprite* sp = Sprite::create(StringUtils::format("Shop/item%d.png",i+1));
	...
	Sprite* enter = Sprite::create("garage/Buy.png");
...
{
	Sprite* myResources = Sprite::create("Shop/myres.png");
