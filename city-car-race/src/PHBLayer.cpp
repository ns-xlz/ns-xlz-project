...
bool PHBLayer::init()
{
	...
	MenuItemImage* resetBt = MenuItemImage::create(
			   "rank/reset.png",	//平时的图片
			   "rank/resetp.png",	//选中时的图片
	...
	Sprite* rank = Sprite::create("rank/rank.png");
	rank->setPosition(Point(270,750));
	this->addChild(rank,2);

	Sprite* jilu = Sprite::create("rank/jilu.png");
	jilu->setPosition(Point(270,750));
	this->addChild(jilu,3);

	Sprite* backBg = Sprite::create("rank/back.png");
	backBg->setPosition(Point(270,400));
	this->addChild(backBg,2);

	Label* zyjl= Label::createWithTTF("\u884c\u9a76\u6700\u8fdc\u8ddd\u79bb:","fonts/FZKATJW.ttf",30);
	...
	for(int i=0;i<5;i++)
	{
		noLabel[i] = LabelAtlas::create(						//创建一个LabelAtlas对象
				StringUtils::format("%d..%d",i+1,record[i]),"gamepic/upNum.png",20,23,'0');
