...
bool ChooseCarLayer::init()
{
    ...
    Sprite* garageSprite = Sprite::create("garage/GarageIcon.png");
    garageSprite->setPosition(270,780);
    this->addChild(garageSprite,1);

    Sprite* garage = Sprite::create("garage/Garage.png");
    garage->setPosition(270,800);
    this->addChild(garage,1);

    for(int i=0;i<8;i++)
    {
    	garageBG[i] = Sprite::create("garage/GarageFrame.png");
		...
		if(buyOrNot == 0)
		{
			lockeSprite[i] = Sprite::create("garage/Locked.png");
		...
		for(int j=0;j<i+1;j++)
		{
			Sprite* dbSprite = Sprite::create("garage/DB.png");
		...
		LabelAtlas* carPriceLabel = LabelAtlas::create(						//创建一个LabelAtlas对象
				StringUtils::format("%d",(i+1)*500),"gamepic/upNum.png",20,23,'0');
		...
		userGoldLabel[i] = LabelAtlas::create(						//创建一个LabelAtlas对象
				StringUtils::format("%d",userGoldCount),"gamepic/upNum.png",20,23,'0');
...
void ChooseCarLayer::refreshMenuSprite()
{
	...
	{
		Sprite* enter = Sprite::create("garage/Enter.png");
	...
	{
		Sprite* enter = Sprite::create("garage/enterMain.png");
		enter->setPosition(270,150);
		this->addChild(enter,2,9);
	}
}
