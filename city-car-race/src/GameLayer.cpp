...
bool GameLayer::init()
{
	...
	if(StartLayer::gameModeLevel == 0)
	{
		...
		Sprite* txtundSP = Sprite::create("gamepic/txtund.png");
		...
		Label* taskLabel = Label::createWithTTF(ss,"fonts/FZKATJW.ttf",48);
	...
	pauseItem = MenuItemImage::create(
			   "gamepic/pauseN.png",						//平时的图片
			   "gamepic/pauseP.png",						//选中时的图片
...
void GameLayer::initSilver()
{
	...
	sfc->addSpriteFramesWithFile("hzjl/silver.plist",
			"hzjl/silver.png");			//将精灵帧文件以及对应的图片添加到内存中
...
{
	SpriteFrameCache* sfc=SpriteFrameCache::getInstance();//获取缓冲精灵帧的实例
	sfc->addSpriteFramesWithFile("hzjl/big_fire.plist",
		"hzjl/big_fire.png");			//将精灵帧文件以及对应的图片添加到内存中
	std::string sa[4]=
	{								//上下左右四段动画8幅图片的名称
...
{
	SpriteFrameCache* sfc=SpriteFrameCache::getInstance();//获取缓冲精灵帧的实例
	sfc->addSpriteFramesWithFile("hzjl/no2h.plist",
		"hzjl/no2h.png");			//将精灵帧文件以及对应的图片添加到内存中
	std::string sa[4]=
	{								//上下左右四段动画8幅图片的名称
		"no21.png","no22.png",
		"no23.png","no24.png"
...
{
	SpriteFrameCache* sfc=SpriteFrameCache::getInstance();//获取缓冲精灵帧的实例
	sfc->addSpriteFramesWithFile("hzjl/newboom.plist",
			"hzjl/newboom.png");			//将精灵帧文件以及对应的图片添加到内存中
	...
	animFrames.clear();
	sfc->addSpriteFramesWithFile("hzjl/boom.plist",
				"hzjl/boom.png");			//将精灵帧文件以及对应的图片添加到内存中
	for(int i=0;i<7;i++)
	{
...
{
	SpriteFrameCache* sfc=SpriteFrameCache::getInstance();//获取缓冲精灵帧的实例
	sfc->addSpriteFramesWithFile("no2/no2.plist",
			"no2/no2.png");			//将精灵帧文件以及对应的图片添加到内存中
	std::string boom[3]=
	{								//上下左右四段动画8幅图片的名称
		"1.png","2.png","3.png"
	};
...
void GameLayer::initSpeedLine()
{
	std::string sa[6]={						//将所有动画图片的路径储存在数组中
		"speedline/speed1.png","speedline/speed2.png",
		"speedline/speed3.png","speedline/speed4.png",
		"speedline/speed5.png","speedline/speed6.png"
	};
...
{
	SpriteFrameCache* sfc=SpriteFrameCache::getInstance();//获取缓冲精灵帧的实例
	sfc->addSpriteFramesWithFile("hzjl/power.plist",
		"hzjl/power.png");			//将精灵帧文件以及对应的图片添加到内存中
	std::string sa[3]=
	{								//上下左右四段动画8幅图片的名称
...
{
	SpriteFrameCache* sfc=SpriteFrameCache::getInstance();//获取缓冲精灵帧的实例
	sfc->addSpriteFramesWithFile("hzjl/firef.plist",
		"hzjl/firef.png");			//将精灵帧文件以及对应的图片添加到内存中
	std::string sa[7]=
	{								//上下左右四段动画8幅图片的名称
...
void GameLayer::initMusic()
{
	CocosDenshion::SimpleAudioEngine::getInstance()
					->preloadBackgroundMusic("mainsound/drive.mp3");
}
...
{
	//下仪表精灵对象
	Sprite* underSprite = Sprite::create("gamepic/under.png");
	underSprite->setPosition(Point(270,107.5));
	this->addChild(underSprite,YBPZORDER);

	Texture2D* texture1 = Director::getInstance()->getTextureCache()->addImage("gamepic/life.png");
	...
	Texture2D* texture2 = Director::getInstance()->getTextureCache()->addImage("gamepic/no2.png");
	...
	rocketItem = MenuItemImage::create(
			   "gamepic/rocketN.png",						//平时的图片
			   "gamepic/rocketP.png",						//选中时的图片
...
void GameLayer::trafficUpdate()
{
	do
	{
		if(djsTime==3)
		{
			...
			if(StartLayer::yxFlag == true)
			{
				CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("mainsound/start1.mp3");
			}
			if(StartLayer::soundFlag==true){
				CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic(	//播放音乐
							"mainsound/drive.mp3",
		...
		if(StartLayer::yxFlag == true)
		{
			CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("mainsound/daojishi.mp3");
		}
...
void GameLayer::enemyCarUpdate()
{
	...
	for(int i=0;i<newX;i++)
	{
		...
		EnemyCar* enemyCar= new EnemyCar(9,130+enemyArray[i]*70,1000,baseSpeed+ChooseCarLayer::currCarNum*2,0);
		enemyCar->refresh();
...
void GameLayer::addStageUpdate()
{
	...
	case 0://金币道具
		...
		break;
	case 1://火箭道具
		itemSprite = Sprite::create("item/item_rocket.png");
		...
		break;
	case 2://护甲道具
		itemSprite = Sprite::create("item/item_power.png");
		itemSprite->setPosition(Point(130+newX,970));
		this->addChild(itemSprite,COINZORDER);
		//加入向量中
		allItemSpriteVCT->push_back(itemSprite);
		...
		break;
	case 3://加速道具
		itemSprite = Sprite::create("item/item_no2.png");
		itemSprite->setPosition(Point(130+newX,970));
		this->addChild(itemSprite,COINZORDER);
		//加入向量中
		allItemSpriteVCT->push_back(itemSprite);
...
void GameLayer::collisionTestPlayerCarAndItem()
{
	...
	while(itemIterator!= allItemSpriteVCT->end())
	{
		...
		if(b == true)
		{
			...
			if(StartLayer::yxFlag == true)
			{
				CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("mainsound/getCoin.mp3");
			}
...
void GameLayer::collisionTestPlayerCarAndEnemyCar()
{
	...
	while(tempCar!= allEnemyCarVCT->end())
	{
		...
		if(b==true)
		{
			if(StartLayer::yxFlag == true)
			{
				CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("mainsound/boom.mp3");
			}
...
{
	ParticleSystemQuad* psq = ParticleSystemQuad::create("lzxt/fire.plist");
